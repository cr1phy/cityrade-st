    ...
    pub fn buy(&mut self, resource: &ResourceType, quantity: u32) -> Result<u32, String> {
        if let Some(item) = self.items.get_mut(resource) {
            if item.quantity >= quantity {
                ...
                return Err(format!("Недостаточно ресурса {} на рынке", resource_to_string(resource)));
            }
        }
        
        Err(format!("Ресурс {} не найден на рынке", resource_to_string(resource)))
    }
...
impl TradeManager {
    ...
    pub fn establish_trade_route(&mut self, 
                                source_city: &str, 
                                target_city: &str, 
                                resource: ResourceType, 
                                quantity: u32) -> Result<(), String> {
        ...
        let price = if let Some(item) = source_market.items.get(&resource) {
            ...
            return Err(format!("Ресурс {} не найден на рынке города {}", 
                              resource_to_string(&resource), source_city));
        };
                ...
                if let (Some(source_city), Some(target_city)) = (
                    ...
                    if source_city.subtract_resources(&route.resource, route.quantity) {
                        ...
                        if let Some(source_market) = self.markets.get_mut(&route.source_city) {
                            if let Some(factor) = source_market.supply_factors.get_mut(&route.resource) {
                                *factor -= 0.02; // Уменьшаем предложение в исходном городе
                            }
                        }
                        
                        if let Some(target_market) = self.markets.get_mut(&route.target_city) {
                            if let Some(factor) = target_market.demand_factors.get_mut(&route.resource) {
