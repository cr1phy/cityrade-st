...
impl Resources {
    pub fn new() -> Self {
        ...
        Resources { resources, production_rate }
    }
    ...
    pub fn update_production(&mut self) {
        for (resource, rate) in &self.production_rate {
            if *rate > 0 {
    ...
    pub fn can_afford(&self, costs: &[(ResourceType, u32)]) -> bool {
        costs.iter().all(|(resource, amount)| self.get(resource) >= *amount)
    }
